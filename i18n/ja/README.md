...
- **🛡️ Hook & MCP のマージ**: リモートの `hooks.json` と `mcp.json` を Antigravity 設定にマージします。Hook 配列は重複排除され、同名の MCP server は warning を表示したうえで新しい設定に置き換えられます。
...
- **フィンガープリントの追跡**: `agy-plugin` は、ダウンロードしたディレクトリの最新の Git `sha` をローカルの `~/.agy-plugin/installed.json` にキャッシュし、marketplace 設定を `~/.agy-plugin/config.json` に保存します。これにより、インテリジェントな差分更新が可能になり、TUI パネルで直感的な視覚的フィードバック（インストール済み vs 更新可能）が提供されます。
